...
- [x] GlitchMark system (Phase 28) — geometric mean × 100 of per-test reference-baseline ratios; schema (4 cols on tech_reviews + reference_score + tech_glitchmark_history) live in prod; pure-function module + 13 vitest cases; recomputeGlitchmark wired into ingest tx atomically with BPR; methodology page transparency section live; v1 locked, GLITCHMARK-01..03+06..08 validated (04 → Phase 29, 05 → FLAG-03)
...
---
*Last updated: 2026-04-27 — Phase 29.2 (Site-Wide Hero Rollout + Methodology Editorial Upgrade) shipped. Ten sequential plans landed: TechHero promoted to a `size?: "compact" | "default" | "tall"` variant prop (200/280/400px) with WCAG 2.5.5 touch-target on the CTA; six remaining /tech/* surfaces (about, reviews, categories hub, compare, benchmarks, blog) now mount TechHero with the prescribed tone/eyebrow/CTA pairs; /tech/about lifted from prose-heavy to editorial — tall amber hero, 43/13/7 stat cards row, MethodologyDisciplineCards (13-card responsive grid with 7 BPR ELIGIBLE badges replacing the flat table), MethodologyMedalLadder (vertical timeline replacing the table), terminal-chrome formula block ($ prefix + hover-only blinking cursor), TABLE-ROW-HOVER (group-hover translate-x-[2px] + GlitchHeading + motion-reduce safe) preserved on the legacy MethodologyDisciplineTable; CategoryTile upgraded to Direction B (80px scaled icon + caption — "{N} reviewed · {N} ranked" combined or "COMING SOON"); /tech/blog stub replaced with proper empty-state copy. Methodology data layer untouched (audited accurate prior phase). 22/22 Playwright assertions pass on desktop + mobile (5 blog-specific + 6 phase-smoke × 2 viewports), plus per-plan specs (~40 individual assertions). Production `pnpm build` exits 0. No GlitchTek typos.*

*2026-04-26 — Phase 29.1 (master leaderboard polish) shipped. Nine sequential plans landed the canonical /tech/rankings + /tech/rankings/[slug] URL structure (legacy /tech/categories/[slug]/rankings now permanentRedirects), the new D-01 sidebar order with Rankings + About surfaces discoverable, the reusable TechHero component on three navigational surfaces (cyan rankings, amber category), top filter bar replacing the left sidebar with all 7 facets always visible, table min-width=1600px engaging horizontal overflow at narrow viewports, mobile Cards↔Table view toggle backed by nuqs (clearOnDefault keeps URLs clean), and the GlitchMark ×10 display convention (1000/1425 instead of 100.00/142.50). The /tech/about page is now the methodology hub with fragment-preserving redirects from /tech/methodology. Production mobile-nav active-state bug (D-23) fixed by isTechPathActive helper that normalises the brand-host rewrite. 43/43 Playwright specs across desktop+mobile projects, 20 vitest cases. Schema unchanged. v3.0 carry-over GLITCHMARK-04 (leaderboard column) now satisfied.*

*2026-04-25 — Phase 28 (GlitchMark system) shipped. Geometric mean × 100 of per-test reference-baseline ratios, computed atomically with BPR on ingest commit, persisted to tech_reviews + tech_glitchmark_history (versioned for future formula iterations). v1 formula transparent on /tech/methodology. Schema live in prod Supabase DB; 13 vitest cases pass. GLITCHMARK-04 (leaderboard column) deferred to Phase 29; GLITCHMARK-05 (review card surface) deferred to Phase 36 (FLAG-03). Operator action D-13 still pending: populate `tech_benchmark_tests.reference_score` per test via SQL UPDATE, then re-ingest a product to populate live scores. Phase 27 (media/video strategy foundation) shipped earlier same day. Polymorphic `media_item` table with `media_kind` enum live in Postgres (4 backfilled rows from existing `portfolio_items.video_url` + `tech_reviews.video_url`). Canonical `<MediaEmbed>` component (youtube-nocookie.com only; hover-preview gated on `(pointer: fine)` + `!prefers-reduced-motion: reduce`; `mode="thumbnailOnly"` for nested-Link cards) is now the only YouTube iframe emitter site-wide — verified via grep. Admin can paste-validate-attach via per-entity edit pages (beat/service/tech_review) and curate `home_feature` rows from `/admin/settings/homepage` with hard-cap-3 read enforcement. Public payoff: `<HomeFeaturedWorkGrid>` 3-up grid (returns null when empty per D-12), `<VideoCard>` thumbnailOnly inside `<Link>`, beat detail conditional "Made by hand" via top-down prop-pass, `<ReviewVideoEmbed>` media_item-first with one-release `video_url` fallback. Portfolio admin Videos section deferred — admin edit page does not exist as standalone surface. Phase 26 (brand-aware auth UI redesign) previously shipped: AUTH-01..32 all validated.*
