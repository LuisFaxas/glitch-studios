...
result: [resolved 2026-04-23 — gap-closed via Plan 17-04. `BenchmarkSpotlight` type extended with bprScore/bprTier/bprDisciplineCount/reviewSlug; `getBenchmarkSpotlight()` populates them from the existing techReviews join; `TechBenchmarkSpotlight` renders BPRMedal/Placeholder between the "Editor's Choice" label and the product title. Live render still pending a published review with populated bpr_score — covered by item 1.]
