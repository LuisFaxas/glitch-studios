...
must_haves:
  ...
  key_links:
    ...
    - from: "src/db/seeds/rubric-v1.1.ts"
      ...
      via: "onConflictDoNothing target on 4 natural-key columns"
...
BPR-eligible disciplines (mode='both', bprEligible=true): cpu, gpu, llm, video, dev, python, games (7 disciplines, 25 rows)
Non-BPR disciplines (mode='ac' or 'battery', bprEligible=false): memory, storage, thermal, wireless, display (mode='ac'); battery_life (mode='battery') (6 disciplines, 14 rows)
...
<task type="auto" tdd="false">
  ...
  <acceptance_criteria>
    ...
    - `pnpm tsx -e "import('./src/lib/tech/rubric-map.ts').then(m => console.log(Object.values(m.RUBRIC_V1_1).filter(v => v.bprEligible).length))"` prints `23` (wait — verify count: cpu 5 + gpu 5 + llm 3 + video 3 + dev 3 + python 2 + games 2 = 23. Correct.)
...
<task type="auto" tdd="false">
  <name>Task 2: Create src/db/seeds/rubric-v1.1.ts — idempotent append-only seed</name>
  ...
  <read_first>
    ...
    - .planning/phases/15-methodology-lock-schema/15-RESEARCH.md §Seed Idempotency Mechanism (lines 487-562)
  ...
  <action>
    ...
    async function main() {
      ...
      // 4. Insert with onConflictDoNothing on natural-key index (template_id, discipline, mode, name).
  ...
  <acceptance_criteria>
    ...
    - `grep -c 'onConflictDoNothing' src/db/seeds/rubric-v1.1.ts` returns at least 1
  ...
  <done>
    Seed script created, produces 39 tech_benchmark_tests rows on first run, zero new rows on subsequent runs. Exit code 0 both runs. No onConflictDoUpdate anywhere. Natural-key UNIQUE index (created in plan 01) is the idempotency guarantor.
  </done>
</task>
...
4. The seed uses onConflictDoNothing on (template_id, discipline, mode, name) and NEVER onConflictDoUpdate
